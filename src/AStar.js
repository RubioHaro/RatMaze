...
class AStar {
  ...
  retrieveOptimalPath(node) {
    ...
    node.through !== this.start ? this.retrieveOptimalPath(node.through) : this.optimalPath.push(this.start)
  }

  eucledianDistance(node) {
    return Math.sqrt(Math.pow(Math.abs(node.row - this.end.row), 2) + 
    Math.pow(Math.abs(node.col - this.end.col), 2))
  }
